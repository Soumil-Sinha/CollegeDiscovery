...
function BestValue({ values, colleges, formatFn, higherIsBetter = true }: {
  ...
  return (
    <>
      {values.map((v, i) => (
        <td key={colleges[i].id} className={cn("px-5 py-3 text-sm text-center", v === bestValue && "bg-green-50 font-semibold text-green-700")}>
          {formatFn(v)}
...
export function CompareTable({ colleges }: CompareTableProps) {
  ...
  const rows = [
    {
      label: "Location",
      render: (c: CollegeCompare) => <td key={c.id} className="px-5 py-3 text-sm text-center text-gray-700">{c.location}</td>,
    },
    {
      label: "Type",
      render: (c: CollegeCompare) => <td key={c.id} className="px-5 py-3 text-sm text-center"><span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-xs font-medium">{c.type}</span></td>,
    },
    {
      label: "Established",
      render: (c: CollegeCompare) => <td key={c.id} className="px-5 py-3 text-sm text-center text-gray-700">{c.established}</td>,
    },
    {
      label: "NAAC Grade",
      render: (c: CollegeCompare) => <td key={c.id} className="px-5 py-3 text-sm text-center font-medium text-gray-900">{c.naacGrade ?? "—"}</td>,
    },
    {
      label: "Rating",
      isMetric: true,
      ...
      render: (c: CollegeCompare) => (
        <td key={c.id} className="px-5 py-3 text-center">
          <div className="flex flex-wrap justify-center gap-1">
            {c.badges.length > 0 ? c.badges.map((b) => <InsightBadge key={b} label={b} />) : <span className="text-xs text-gray-400">—</span>}
          </div>
        </td>
      ),
    },
    {
      label: "Top Course",
      render: (c: CollegeCompare) => (
        <td key={c.id} className="px-5 py-3 text-sm text-center text-gray-700">
          {c.courses[0]?.name ?? "—"}
        </td>
      ),
    },
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-200">
            <th className="px-5 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide w-36">
              Criteria
            </th>
            {colleges.map((c) => (
        ...
        <tbody className="divide-y divide-gray-100 bg-white">
          {rows.map((row, idx) => (
            <tr key={idx} className="hover:bg-gray-50 transition-colors">
              <td className="px-5 py-3 text-sm font-medium text-gray-600">{row.label}</td>
