    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <p className="text-xs uppercase tracking-widest text-gray-400 mb-1">Dashboard</p>
            <h1 className="font-serif text-4xl text-gray-900 leading-tight">
              Welcome, <span className="italic">{session.user.name?.split(" ")[0]}</span>
            </h1>
            <p className="text-sm text-gray-500 mt-1"><span className="numeric">{savedColleges.length}</span> saved colleges</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <h2 className="font-serif text-2xl text-gray-900">Saved Colleges</h2>

            {savedColleges.length === 0 ? (
              <div className="bg-white border border-dashed border-gray-300 rounded-xl p-12 text-center fade-in">
                ...
                <p className="text-gray-500 text-sm">You haven&apos;t saved any colleges yet.</p>
              ...
              <div className="space-y-3">
                      ...
                      <CardBody>
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <Link href={`/colleges/${college.slug}`} className="font-medium text-gray-900 hover:text-indigo-600 transition line-clamp-1">
                              {college.name}
                            </Link>
                            <p className="text-xs text-gray-500 mt-0.5">{college.city}, {college.state}</p>
                          </div>
                          <div className="hidden sm:flex items-center gap-6 text-center flex-shrink-0">
                            <div>
                              <p className="text-xs text-gray-500">Fees</p>
                              <p className="text-sm font-semibold text-gray-900">{formatFees(college.totalFees)}</p>
                            </div>
                            <div>
                ...
                {savedColleges.length >= 2 && (
                  <Link
                    ...
                    className="block w-full text-center text-sm text-indigo-600 border border-indigo-200 hover:bg-indigo-50 px-4 py-3 rounded-xl transition font-medium"
                  >
          ...
          <div className="space-y-6">
            {savedColleges.length > 0 ? (
              ...
              <Card>
                <CardBody className="text-center py-8 text-gray-400 text-sm">
