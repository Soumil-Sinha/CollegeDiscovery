        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1">
              ...
              <h1 className="font-serif text-4xl text-gray-900 mb-2 leading-tight">{college.name}</h1>
              <p className="text-gray-500 text-sm mb-3 inline-flex items-center gap-1.5">
              ...
              <p className="text-gray-600 text-sm leading-relaxed max-w-2xl">{college.description}</p>
            </div>
            ...
            <div className="flex flex-col gap-2 flex-shrink-0">
              ...
              <Link
                ...
                className="text-sm text-center text-indigo-600 border border-indigo-200 hover:bg-indigo-50 px-4 py-2 rounded-lg transition"
              >
                + Add to Compare
              </Link>
              {college.website && (
                <a
                  ...
                  className="text-sm text-center text-gray-600 border border-gray-200 hover:bg-gray-50 px-4 py-2 rounded-lg transition"
                >
                  Visit Website ↗
                </a>
              )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              {[
                ...
                <Card key={label} className="transition-all duration-300 hover:border-gray-300">
                  <CardBody className="text-center py-4">
                    ...
                    <p className="numeric text-2xl text-gray-900">{value}</p>
                  </CardBody>
                </Card>
              ))}
            </div>
            ...
            <Card>
              <CardHeader>
                <h2 className="font-serif text-xl text-gray-900">Placement Trends</h2>
                <p className="text-xs text-gray-500 mt-0.5">Salary trends over 3 years (in Lakhs per annum)</p>
              </CardHeader>
              <CardBody>
            ...
            {college.branchPlacementStats.length > 0 && (
              <Card>
                <CardHeader>
                  <h2 className="font-serif text-xl text-gray-900">Branch-wise Placement Breakdown</h2>
                  <p className="text-xs text-gray-500 mt-0.5">Compare placement stats across CSE, ECE, ME, CE & EE</p>
                </CardHeader>
                <CardBody>
            ...
            <Card>
              <CardHeader>
                <h2 className="font-serif text-xl text-gray-900">Courses Offered</h2>
              </CardHeader>
                  ...
                  {college.courses.map((course) => (
                    <div key={course.id} className="flex items-center justify-between px-5 py-3">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{course.duration} years · {course.seats} seats</p>
                      </div>
                      <p className="text-sm font-semibold text-gray-700">{formatFees(course.fees)}</p>
                    </div>
                  ))}
                </div>
              </CardBody>
            ...
            <Card>
              <CardHeader>
                <h2 className="font-serif text-xl text-gray-900">Latest News</h2>
                <p className="text-xs text-gray-500 mt-0.5">Recent coverage about {college.name}</p>
              </CardHeader>
              <CardBody>
          ...
          <div className="space-y-6">
            ...
            {latestStat && (
              <Card>
                <CardHeader>
                  <h3 className="font-serif text-lg text-gray-900">Placements 2024</h3>
                </CardHeader>
            ...
            <Card>
              <CardHeader>
                <h3 className="font-serif text-lg text-gray-900">Top Recruiters</h3>
              </CardHeader>
              <CardBody>
                <div className="flex flex-wrap gap-2">
                  {allTopRecruiters.map((company) => (
                    <span key={company} className="text-xs bg-gray-100 text-gray-700 px-2.5 py-1 rounded-full font-medium">
                      {company}
                    </span>
                  ))}
                </div>
            ...
            <Card>
              <CardHeader>
                <h3 className="font-serif text-lg text-gray-900">Year-wise Placements</h3>
              </CardHeader>
              <CardBody className="p-0">
                <div className="divide-y divide-gray-100">
                  {[...college.placementStats].reverse().map((stat) => (
                    <div key={stat.id} className="px-5 py-3">
                      <div className="flex justify-between items-center mb-1.5">
                        <span className="text-sm font-medium text-gray-900">{stat.year}</span>
                        <span className="text-xs text-gray-500">{stat.placementRate}% placed</span>
                      </div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">Avg: {formatSalary(stat.avgSalary)}</p>
                    </div>
                  ))}
                </div>
              </CardBody>
